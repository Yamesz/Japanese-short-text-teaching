...
function syncStoryAudioControls(){const savedRate=parseFloat(localStorage.getItem('sakura_speech_rate')||'1');document.querySelectorAll('.speed-control .speed-btn').forEach(b=>b.classList.toggle('active',parseFloat(b.dataset.rate)===savedRate));}
function setStorySpeechRate(rate,btn){setSpeechRate(rate,btn);syncStoryAudioControls();if(!player)return;player.rate=rate;if(player.state==='playing'){player.isChangingSpeed=true;player.synth.cancel();setTimeout(()=>{player.isChangingSpeed=false;if(player.state==='playing')player._playNext();},120);}}
function switchLevel(level){if(player)player.stop();currentLevel=level;player=new KaraokePlayer(level);document.querySelectorAll('.level-btn').forEach(b=>b.classList.toggle('active',b.dataset.level===level));document.querySelectorAll('.level-content').forEach(c=>c.classList.toggle('active',c.id===`level-${level}`));const box=document.querySelector(`#level-${level} .lang-toggle input`);if(box)box.checked=false;toggleTranslation({checked:false});syncStoryAudioControls();}
...
document.addEventListener('DOMContentLoaded',()=>{document.getElementById('levels-root').innerHTML=['n5','n4','n3'].map(renderLevel).join('');switchLevel('n5');syncStoryAudioControls();initArticleLearnedState('prog-story-04');});
